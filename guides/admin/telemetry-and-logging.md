...
This means an admin does not have to watch logs to learn that a system job failed — the failure surfaces as a notification. The routing model and the user-facing notification behavior are documented in [Notifications](../user/notifications.md) and, for the mechanics, the developer [Notifications](../../references/platform/notifications.md) reference.
