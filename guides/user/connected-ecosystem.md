...
An agent is a deterministic workflow expressed as an [Open Agent Specification (OAS) Flow](../../references/platform/open-standards.md) declarative file. It declares its inputs, its sub-agents, its tools, and its execution graph. The platform runs that flow in a WayFlow (Cinatra's OAS Flow agent runtime) Python sidecar; the chat UI, the run page, and any Model Context Protocol (MCP) client all see the same execution stream.

Agents come from three places:

- **The marketplace.** Installed from the shared registry or from another Cinatra instance. See the [Registry and marketplace](../admin/marketplace.md) admin reference and [Cross-instance collaboration](cross-instance-collaboration.md) for the user-side workflow.
...
The composition rules — how one agent can declare another agent as a sub-agent dependency, how versions resolve, how the runtime hands off — are formalised in [Open standards in Cinatra](../../references/platform/open-standards.md).
...
For the data-model details — typed identifiers, registration, dual-write hooks — see [The objects layer](../../references/platform/objects-layer.md).
