...
- Check the [Configuration](configuration.md) reference for the variable that controls the symptom you are seeing.
- For agent-runtime-specific issues, look at the WayFlow container logs: `docker compose logs -f wayflow`.
- For the open standards behind the runtime, see [Open standards in Cinatra](../../references/platform/open-standards.md). For the MCP architecture, see the [MCP Guide](../../references/mcp/README.md).
