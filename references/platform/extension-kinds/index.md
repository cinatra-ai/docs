...
| **artifact** | A semantic content type with matcher/authoring skills. | A descriptor package declaring an `artifact` block + a classifier `SKILL.md`. | No (declarative). | [Authoring artifact extensions](authoring-artifact-extensions.md) |
...
- **Teaching the platform a new work-product type** that the LLM should recognize and classify (a contract, a brief, a design doc)? → **artifact**. A metadata descriptor plus a classifier skill.
