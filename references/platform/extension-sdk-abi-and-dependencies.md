# Extension SDK ABI and dependencies

The `@cinatra-ai/sdk-extensions` package is the **frozen author-facing ABI**
every Cinatra extension (agent, connector, skill, artifact, workflow) builds
against.
...
The SDK ABI is exported as `SDK_EXTENSIONS_ABI_VERSION`. The MAJOR bumps on any
breaking change to the author-facing contract (ports, lifecycle, manifest
fields). The `2.x` line added the `telemetry` host port for metered connectors.
...
runbook: [The runtime-store `serverEntry` contract](extension-server-entry-artifact.md).
...
the same engine, options, and shared ledger the host core uses (the
schema-migration framework). The HOST runs them; the extension never receives
a DB handle.
...
> The legacy declarative JSON-DSL field (`cinatra.migrations`) is **retired**
...
the schema-migration framework). The contract:
