...
> **Renderer boundary — moving off host-side registration.** Historically the host owned every type's view: the `renderers` above were registered host-side and core carried per-type UI knowledge. That inverts the intended boundary — **core owns dispatch, the shell, and the never-blank floor; an artifact extension ships its own type's view.** For artifact **types**, the `detail`/`preview` view ships from the type's own extension through the `cinatra.artifact.ui` block and resolves through the generated renderer map (`src/lib/generated/artifact-renderers.ts`) plus org-scoped arbitration registries (`src/app/artifacts/[id]/renderer-resolution.ts`) — **not** a host-side `renderers` registration keyed by a concrete type id. The **`listRow` slot is part of the active `cinatra.artifact.ui` v1 slot enum** alongside `detail` and `preview` (`ARTIFACT_UI_SLOTS` in `packages/sdk-extensions/src/artifact-contract.ts`): an extension may ship the compact row renderer the artifacts library resolves for a row it claims. **`card` and `inline` are reserved** (`ARTIFACT_UI_RESERVED_SLOTS`) and are rejected by the closed v1 enum, so a `card` view stays host-owned through the object-type `renderers` block above. Core treats type identity as **opaque**: a CI boundary gate (`scripts/audit/artifact-ui-boundary-gate.mjs`) **fails** production-core code that depends on, compares, looks up, imports, or transforms by a concrete extension-owned presentation identity for type-specific presentation, against a shrink-only baseline that enumerates every remaining keyed arm with its disposition. Treat the host-side `renderers` block above as the **transitional** shape for types that have not migrated, not the end-state contract.
