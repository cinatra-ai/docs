...
extension packages, the production marketplace model. The runtime package store
activates **BUILT artifacts only** — a TypeScript source mirror is refused
loudly at install (materialize) time, never deferred to an opaque activation
failure.
...
- **Self-contained bundle (inline mode — the default).** The store does not
  ...
  for the declare-and-closure alternative (a future closure-mode design).
...
`cinatra.dependencyMode` (a future closure-mode design) selects how a
package's npm LIBRARY dependencies reach the runtime store. The builder reads
it from the manifest; the `--mode` CLI flag overrides it for TESTS ONLY.
...
This gate lives OUTSIDE this repo, in the release pipeline (`release-submit.mjs`,
published alongside `build-server-entry.mjs`) that the reusable extension
release workflow fetches (and the marketplace wave-runner shells out to). It
refuses to submit a tarball whose PACKED manifest (read from the tarball
bytes) violates the contract. Message head:
...
| --- | --- | --- |
| Acquisition | `cinatra-required-extensions.lock.json` (SHA-pinned source tarballs) baked into the image build; dev trees clone workspace members ([pinning doc](../../guides/developer/clone-on-demand.md)) | Marketplace install: SRI-verified tarball → materialized store dir → trusted anchor |
...
the retired capability id. Concrete case: a retired delegating-adapter
capability id (after the zero-floor IoC enforcement landed) — a pre-re-point
digest resolves it to nothing and throws when first used; every current
package resolves the connector-authored capability surface directly.
...
1. **Sequence it correctly.** A first-party connector refresh is only
   ...
   (the built-artifact republish wave). Refreshing earlier hits the
...
- [Clone-on-demand pinning](../../guides/developer/clone-on-demand.md) — how the
  static path acquires and pins extension SOURCE.
- [Extension SDK ABI and dependencies](extension-sdk-abi-and-dependencies.md) —
  the author-facing SDK ABI, including `register(ctx)` and the manifest contract.
