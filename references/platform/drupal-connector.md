...
| `src/lib/drupal-mcp-client.ts` | `callDrupalMcp` — Model Context Protocol (MCP) SDK `Client` + `StreamableHTTPClientTransport` |
...
| `src/app/api/drupal-widget/chat/route.ts` | CORS + Bearer gate + agent-to-agent (A2A) protocol blocking dispatch + server-sent events (SSE) |
...
The `drupal/mcp_tools ^1.0@beta` module uses non-obvious tool names. The full mapping is in `packages/connector-drupal/AGENTS.md`. The most important quirk: **there is no get-by-ID tool** — `mcp_tools_search_content` is used as a proxy for `drupal_node_get`.
