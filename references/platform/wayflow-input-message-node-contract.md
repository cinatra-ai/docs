...
5. **Cross-link to this ADR:** `metadata.cinatra.docsRef`: relative path to this ADR (e.g. `"references/platform/wayflow-input-message-node-contract.md"`) — used by future migration tooling to grep every gate that follows this convention.
...
Each `oas.json` carries a `metadata.cinatra.docsRef: "references/platform/wayflow-input-message-node-contract.md"` cross-link pointing at this document.
...
- [ ] Add this ADR to your cross-links: `metadata.cinatra.docsRef: "references/platform/wayflow-input-message-node-contract.md"`.
