...
For the deeper model of how HITL gates work, see [Human-in-the-loop by design](../user/human-in-the-loop.md).
...
The streaming AG-UI event channel exists alongside MCP. It's reachable at `{CINATRA_BASE_URL}/api/a2a?taskId={runId}` as Server-Sent Events. See [Open standards in Cinatra](../developer/open-standards.md) for the event types (`RUN_STARTED`, `TEXT_MESSAGE_CONTENT`, `TOOL_CALL_START`, `INTERRUPT`, `STATE_SNAPSHOT`, `RUN_FINISHED`).
...
- The HITL contract on the user side: [Human-in-the-loop by design](../user/human-in-the-loop.md)
- The streaming event channel: [Open standards in Cinatra](../developer/open-standards.md)
