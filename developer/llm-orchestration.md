...
The token request must include `resource: getLocalMcpServerUrl("/api/mcp")` (RFC 8707). Without it, Better Auth (the auth server library Cinatra uses) issues an opaque token, which cannot be verified by JWKS. See `docs/developer/mcp-patterns.md` — LLM provider access section for full details.
