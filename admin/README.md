...
If you are an end user, see the [User Guide](../user/README.md). If you install or operate Cinatra, see the [Hosting Guide](../hosting/README.md). If you work on the code, see the [Developer Guide](../developer/README.md). For the Model Context Protocol (MCP) server, see the [MCP Guide](../mcp/README.md).
...
- **Extensions** — the installed-extensions lifecycle: install, archive, restore, force-delete, plus GitHub-URL skill upload. See the [Extensions reference](../developer/extensions.md) for protocol-level details on installation and origin tracking.
...
Every install, update, archive, restore, force-delete, and promotion is recorded in the audit log with actor, resource, action, and time. See [Security](../developer/security.md) for the full audit-trail model.
...
- The platform's authorization model: [Security](../developer/security.md)
- The MCP primitives that map to admin actions: [Primitives](../mcp/primitives.md)
