...
- **Encryption key.** Cinatra encrypts provider keys and connector credentials at rest. In development the key is generated and persisted automatically; in production the operator supplies it as an environment variable. This is an operator-coordinated decision — see [Security](../developer/security.md).
...
- **Audit.** The install and any privileged actions you took are recorded in the audit log. Every install, update, archive, restore, and grant writes an audit row naming you as the actor. See [Permissions](permissions.md) and [Security](../developer/security.md).
