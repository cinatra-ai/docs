...
If you are an end user, see the [User Guide](../user/README.md). If you administer the `/configuration/*` area, see the [Admin Guide](../admin/README.md). If you work on the code, see the [Developer Guide](../developer/README.md). For the Model Context Protocol (MCP) server (external and internal), see the [MCP Guide](../mcp/README.md).
...
- **WayFlow Python sidecar** — hosts the Open Agent Specification (OAS) Flow runtime that executes agent runs. The Next.js app calls it over agent-to-agent (A2A) protocol. The sidecar is replaceable by any OAS-compliant runtime; see [Architecture](../developer/architecture.md) for the integration contract.
...
- The architecture you're hosting: [Architecture](../developer/architecture.md)
