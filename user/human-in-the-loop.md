...
- Resume from a **remote MCP client** — Claude Code, ChatGPT connectors, an automation script — by calling the `agent_run_resume` primitive with the run ID and the human's response. See [Agent runs over MCP](../mcp/agent-runs-over-mcp.md).
...
- The streaming event contract HITL rides on: [Open standards in Cinatra](../developer/open-standards.md)
